...
@Component({
  ...
  styleUrl: './home-cms.component.css'
})
export class HomeCmsComponent implements OnInit{
  ...
  formDetails = {banner_id: 0, loc_id: 0, loc_name: '' , title: '',
  image_path: [] as string[], remove_image_path: [] as string[], status: true, created_on: '2025-08-20', created_by:'Admin',updated_on:'2025-08-20',updated_by:'Admin' };
  details: any[] = [];
  buttonName: any;
  imageApiUrl: any;
  locations: any[] = [];
...
private configService: ConfigService){
  ...
  onSubmit(form: NgForm) {
    ...
    if (!this.formDetails.banner_id || this.formDetails.banner_id === 0) {
      // Add new location
      ...
      this.api.post('Home/CreateHome', this.formDetails, false).subscribe({
      next: (res: any) => {
        if(res.isSuccessful){
          this.getHomeDetails();
          this.reset(form);
      ...
      this.api.update('Home/UpdateHome', this.formDetails, false).subscribe({
      next: (res: any) => {
        if(res.isSuccessful){
          this.getHomeDetails();
          this.reset(form);
   ...
   getHomeDetails(){
     this.api.getAll('Home/GetHome', false).subscribe({
      next: (res: any) => {
  ...
  reset(form?: NgForm) {
    this.formDetails = { banner_id: 0, loc_id: 0, loc_name: '', title: '',
    image_path: [] as string[],remove_image_path: [] as string[], status: true, created_on: '2025-08-20', created_by:'Admin',updated_on:'2025-08-20',updated_by:'Admin' };
  ...
  deleteItem(item: any) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this.api.deleteById('Home', item.banner_id, false).subscribe({
          next: (res: any) => {
            if (res.isSuccessful) {
              this.toastr.success(res.message);
              this.reset();
  ...
  onStatusChange(item: any) {
    ...
    this.api.updateById('Home/ArchiveHome', item.banner_id, false).subscribe({
      next: (res: any) => {
        if (res.isSuccessful) {
          this.toastr.success(res.message);
          this.reset();
